...
<%@ include file="header.jsp" %>
	...
	<h1>reviewWrite</h1>
		...
		여행 <select id="planId" name="planId" data-planid="${temp.planId}">
		...
		별점 <select id="stars" name="stars" data-stars="${temp.stars}" >
			<c:forEach var="i" begin="0" end="10" step="1">
				<option value="${10/2.0 - i/2.0}">${10/2.0 - i/2.0}</option>
			</c:forEach>
		</select><br />
		<input type="text" name="title" id="title" data-title="${temp.title}" placeholder="제목을 입력해주세요"/><br />
		<textarea id="summernote" name="content" data-content="${temp.content}"></textarea>
		<br />
